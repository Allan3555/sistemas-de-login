...
# def para verificar se a senha nova tem mais de 8 digitos e tem que coincidir com a senha_verify
    ...
    def Criar(self, email_novo, senha_novo):
        senha = self.gerar_hash(self.senha_novo)
        self.arq = open(ARQUIVO, 'a')
        self.arq.write(f'{self.email_novo};{senha}\n')
        self.arq.close()

    # verificar no login se o email e a senha estao no txt
    def logar(self, email_l, senha_l):
        senha = self.gerar_hash(self.senha_l)
        self.arq = open(ARQUIVO, 'r')
        leitura = [line.strip() for line in self.arq.readlines()]
        try:
            if leitura.index(f'{self.email_l};{senha}') == True:
